...
def normalize_article(title: str) -> str:
    """
    Convertit le format bibliographique MovieLens en titre naturel.
    'Dark Knight, The (2008)' → 'The Dark Knight (2008)'
    'Outsiders, The (1983)'   → 'The Outsiders (1983)'
    'Dîner de cons, Le (1998)'→ 'Le Dîner de cons (1998)'
    """
    if not isinstance(title, str):
        return title
    m = re.match(
        r"^(.*?),\s*(The|A|An|Les|Le|La|L')\s*(\(\d{4}\))?$",
        title, re.IGNORECASE
    )
    if m:
...
async def fetch_tmdb_by_search(session, semaphore, movie: dict, cache: dict) -> dict | None:
    ...
    title_raw = normalize_article(title_raw)          # ← AJOUT : "X, The" → "The X"
    ...
    if record and real_tmdb_id != original_id:
        cache.pop(str(real_tmdb_id), None)
        record = dict(record)
        record["real_tmdb_id"] = real_tmdb_id
        record["tmdbId"]       = original_id
        cache[str(original_id)] = record
